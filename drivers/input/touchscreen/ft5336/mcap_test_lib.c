...
static void focal_msleep(int ms) { msleep(ms); }
...
void GetTestParam(void) {
  ...
  g_testparam.rawdata_min =
      GetParamValue(Section_BaseSet, Item_RawData_Min, 5500);
  g_testparam.rawdata_max =
      GetParamValue(Section_BaseSet, Item_RawData_Max, 9500);
  g_testparam.diffdata_min =
      GetParamValue(Section_BaseSet, Item_VolDifferData_Min, 10);
  g_testparam.diffdata_max =
      GetParamValue(Section_BaseSet, Item_VolDifferData_Max, 500);
  ...
  if (g_testparam.Set_Vol > 0) {
    ...
    g_testparam.changevol_level =
        GetParamValue(Section_BaseSet, Item_Vol_Level, 2);
  }
  ...
  for (i = 0; i < TX_NUM_MAX; i++) {
    for (j = 0; j < RX_NUM_MAX; j++) {
      ...
      g_testparam.valid_node[i][j] =
          GetParamValue(Section_INVALID_NODE, str_tmp, 1);
    }
  }
...
int SetDriverVol(unsigned char vol) { return WriteReg(Reg_VolAddr, vol); }
      ...
      for (LineNum = 0; LineNum < iTxNum; LineNum++) {
        ...
        if (ReCode >= 0) {
          ...
          for (j = 0; j < loop; j++) {
            ...
            if (ReCode >= 0) {
              for (i = 0; i < (len >> 1); i++) {
                RawData[LineNum][i + j * mtk_readlen / 2] =
                    (short)((unsigned short)(rrawdata[i << 1]) << 8) +
...
void Ft5336_TestRawDataAndDiff() {
  ...
  for (i = 0; i < iTxNum; i++) {
    for (j = 0; j < iRxNum; j++) {
      ...
      if (rawdata[i][j] < min_value || rawdata[i][j] > max_value) {
        bRawdataTest = false;
        FTS_DBG("rawdata test failure. min_value=%d max_value=%d "
                "rawdata[%d][%d]=%d \n",
  ...
  for (i = 0; i < iRxNum; i++) {
    ...
    } else if (Min_Offset < ucChangeCB &&
               Max_Offset < 31 - maxTxOffset - ucChangeCB) {
  ...
  for (i = 0; i < iTxNum; i++) {
    for (j = 0; j < iRxNum; j++) {
      diffdata[i][j] =
          diffdata[i][j] - rawdata[i][j] +
          (bs_diffdata[i][j] * (m_TxOffset[i] + m_RxOffset[j] + 3) *
           (start_vol - end_vol)) /
              ((16 + start_vol) * change_rxoffset[j]);
      ...
      if (focal_abs(diffdata[i][j]) < min_value ||
          focal_abs(diffdata[i][j]) > max_value) {
        bRawdataTest = false;
        // if(i == 0)
        //{
        FTS_DBG("diffdata test failure. min_value=%d max_value=%d "
                "diffdata[%d][%d]=%d ",
